...
def kmeans_segmentation_fast(image, k=3):
    ...
    # cv.imshow("K-Means", np.uint8(label_img))
    # cv.imshow("Mapped", np.uint8(mapped))
    # cv.waitKey(0)
    return mapped
...
while cap.isOpened():
    ...
    #Green Rectangles
    cv.rectangle(vis_overlay, (center_x - 100, detection_y - 66), (center_x, detection_y + 66), (0, 255, 0), 2)
    cv.rectangle(vis_overlay, (center_x, detection_y - 66), (center_x + 100, detection_y + 66), (0, 255, 0), 2)
    ...
    if object_detected:
        ...
        # Reduce obj count, so that it realistically slows down and doesn't just immediately go to breaking to account
        # for sway in camera
    ...
    if all(v is not None for v in [left_bottom, right_bottom, left_upper, right_upper]):
        # Offset inward by 10 pixels for more accurate lane boundary visualization
        left_bottom += 10
        right_bottom -= 10
        left_upper += 10
        right_upper -= 10
